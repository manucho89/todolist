...
class TasksActivity : AppCompatActivity() {
    ...
    private val speechRecognizerLauncher = registerForActivityResult(
        ...
        if (result.resultCode == RESULT_OK) {
            val spokenText = result.data?.getStringArrayListExtra(RecognizerIntent.EXTRA_RESULTS)?.get(0) ?: return@registerForActivityResult
            if (spokenText.isNotBlank()) {
                val newTask = Task(title = spokenText, listId = listId)
    ...
    private fun setupUI() {
        ...
        adapter = TaskAdapter(
            onTaskChecked = { task -> taskViewModel.updateTask(task) },
            onTaskDelete = { task -> showDeleteConfirmation(task) },
            onTaskPriorityChanged = { task -> taskViewModel.updateTask(task) },
            onTaskLongClick = { task -> showTaskOptionsDialog(task) }  // ⭐ NUEVO
        )
    ...
    private fun setupSwipeToDelete() {
        val swipeHandler = object : SwipeToDeleteCallback(this) {
            override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
                ...
                Snackbar.make(recyclerView, "Tarea eliminada", Snackbar.LENGTH_LONG).setAction("DESHACER") {
    ...
    private fun observeTasks() {
        if (listId == -2) {
            taskViewModel.getAllImportantTasks().observe(this) { tasks ->
                Log.d("TasksActivity", "Tareas importantes recibidas: ${tasks.size}")
                currentTasks = tasks
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item.itemId) {
            R.id.action_share -> {
                if (listId == -2) {
                    Toast.makeText(this, "No se puede compartir esta vista especial", Toast.LENGTH_SHORT).show()
                } else {
                    shareList()
                }
                true
    ...
    // ⭐ NUEVO: Diálogo de opciones al hacer long click
    private fun showTaskOptionsDialog(task: Task) {
        val options = arrayOf("✏️ Editar tarea", "🗑️ Eliminar tarea")
    ...
    // ⭐ NUEVO: Diálogo para editar tarea
    private fun showEditTaskDialog(task: Task) {
        val editText = EditText(this)
        editText.setText(task.title)
        editText.setSelection(task.title.length)
    ...
    private fun deleteCompletedTasks() {
        ...
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Eliminar") { _, _ ->
                ...
                completedTasks.forEach { task -> taskViewModel.deleteTask(task) }
                Snackbar.make(recyclerView, "${completedTasks.size} tarea(s) eliminada(s)", Snackbar.LENGTH_LONG).setAction("DESHACER") {
                    deletedTasks.forEach { task -> taskViewModel.insertTask(task) }
                }.show()
            }
            .setNegativeButton("Cancelar", null)
            .show()
    ...
    private fun shareList() {
        ...
        val sharedTasks = currentTasks.map { task -> SharedTask(task.title, task.isCompleted) }
    ...
    private fun showImportDialog(sharedList: SharedTaskList) {
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Importar") { _, _ -> importList(sharedList) }
    ...
    private fun importList(sharedList: SharedTaskList) {
        val newList = TaskList(name = sharedList.listName, color = sharedList.listColor)
        listViewModel.insertListWithCallback(newList) { newListId ->
            sharedList.tasks.forEach { sharedTask ->
                val task = Task(title = sharedTask.title, isCompleted = sharedTask.isCompleted, listId = newListId.toInt())
                taskViewModel.insertTask(task)
            }
            runOnUiThread {
        ...
        val dialog = AlertDialog.Builder(this)
            ...
            .setPositiveButton("Crear") { _, _ ->
                ...
                if (taskTitle.isNotBlank()) {
                    val newTask = Task(title = taskTitle, listId = listId)
